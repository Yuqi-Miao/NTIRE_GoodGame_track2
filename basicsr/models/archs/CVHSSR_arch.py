...
class CHIMB(nn.Module):  # GFDN
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, expansion_factor=2,drop_out_rate=0.):
        super().__init__()
        ...
        self.gamma = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        # GDFN
        hidden_channels = int(c * expansion_factor)
        self.project_in = nn.Conv2d(c, hidden_channels * 2, kernel_size=1, bias=False)
        self.conv = nn.Conv2d(hidden_channels * 2, hidden_channels * 2, kernel_size=3, padding=1,
                              groups=hidden_channels * 2, bias=False)
        self.project_out = nn.Conv2d(hidden_channels, c, kernel_size=1, bias=False)
...
class CHIMB_last(nn.Module):  # GFDN
    def __init__(self, c, DW_Expand=1, FFN_Expand=1, expansion_factor=1,drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0, stride=1, groups=1,
                               bias=True)
...
class CHIMBSR_last(nn.Module):

    def __init__(self, c, fusion=False):
        super().__init__()
        # self.blk = MAB(c)
        self.blk = CHIMB_last(c)
        self.fusion = CVIM(c) if fusion else None
...
class CVHSR(nn.Module):
    ...
    def __init__(self, up_scale=4, width=48, num_blks=24, img_channel=3, drop_path_rate=0., drop_out_rate=0.,
        ...
        #self.body.add_module("module_name",CHIMBSR_last(width,fusion=True))
...
if __name__ == '__main__':
    ...
    print('flops and mac', macs, params)
